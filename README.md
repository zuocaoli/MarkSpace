...
- **目录管理**：左侧目录树，后台线程扫描、只读浏览（展开/折叠、点击打开、当前文件选中高亮、手动刷新）
...
**v1 已取舍**：编辑器为纯文本（无语法高亮）；目录树只读（无新建/改名/删除）；无编辑器↔预览滚动同步；无外部文件变更监听；窗口使用默认标题栏。

**规划中的后续迭代**：

- Markdown 语法高亮（`gpui-component` 内置 tree-sitter + 高亮器，一行开关即可接入）
- 目录树内新建 / 重命名 / 删除文件，并自动刷新
